...
| `Assisted-by:` | `<role> <model>` | the writing agent | **always** — the queryable audit trail; **never** `Co-Authored-By:` (a model can't hold copyright or sign a CLA). **Session-authored in the `describe` message, derived from the dispatch record** (the agent's role + the model id passed at dispatch) — never a jj config, never recalled from memory |
